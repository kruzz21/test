/*
  # Streamlined Appointment System Migration

  1. New Tables
    - `calendar_slots` - Defines recurring availability patterns (day of week, time ranges)
    - `blocked_slots` - Allows blocking specific time slots for maintenance, holidays, etc.

  2. Enhanced Appointments
    - Add patient_id column for better patient tracking
    - Improved constraints for data validation

  3. Functions
    - `generate_slots_for_date()` - Generates available time slots for a specific date
    - `get_calendar_data()` - Returns calendar data for admin panel
    - `get_available_slots_enhanced()` - Enhanced slot availability for user booking
    - `block_time_slots()` - Function to block specific time ranges
...
BEGIN
  ...
  WHILE current_date <= end_date LOOP
    ...
    RETURN QUERY
    SELECT 
      current_date::text as slot_date,
