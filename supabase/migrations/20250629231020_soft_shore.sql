/*
  # Enhanced Calendar Data Function

  1. Functions
    - Creates `is_admin()` function for admin role checking
    - Creates `get_calendar_data_enhanced()` function for calendar management
    
  2. Security
    - Functions use SECURITY DEFINER for proper access control
    - Grants appropriate permissions to public and authenticated users
*/
...
DROP FUNCTION IF EXISTS get_calendar_data_enhanced(date, date);
...
BEGIN
  ...
  WHILE loop_date <= end_date LOOP
    ...
    FOR slot_record IN 
      ...
      WHERE cs.day_of_week = EXTRACT(DOW FROM loop_date)
