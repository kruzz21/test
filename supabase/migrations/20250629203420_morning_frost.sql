/*
  # Fix Authentication Functions and Dependencies

  1. Drop dependent policies first
  2. Drop and recreate all authentication functions
  3. Recreate the policies that depend on is_admin()
  4. Grant proper permissions

  This migration resolves the dependency error by properly managing
  the order of operations when updating functions used by RLS policies.
*/

-- Enable required extensions
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Drop dependent policies first to remove dependencies on is_admin()
...
-- Now we can safely drop all functions
...
CREATE OR REPLACE FUNCTION verify_password(password_input text, hashed_password text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION is_account_locked(email_input text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Recreate policies that depend on is_admin() function
...
GRANT EXECUTE ON FUNCTION log_audit_event TO authenticated, anon, service_role;
