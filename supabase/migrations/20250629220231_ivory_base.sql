/*
  ...
  2. Enhanced Functions
    - `generate_slots_for_date()` - Core slot generation logic
    - `get_calendar_data()` - Admin calendar view
    - `get_available_slots_enhanced()` - User booking interface
    - `block_time_slots()` - Block specific times
  ...
  4. Default Configuration
    - Monday-Friday, 9 AM-5 PM with lunch break
...
-- Only create this index if appointment_slots table exists
DO $$
BEGIN
...
DROP FUNCTION IF EXISTS generate_slots_for_date(date);
...
BEGIN
  ...
  SELECT 
    ...
    gs.slot_time::text as slot_time_value,
